...
SC.chart.pie = function () {
	...
	var pie = d3.layout.pie()
		.sort(null)
		.startAngle(0 * Math.PI) // startAngle where chart begins (in radians)
		.endAngle(2 * Math.PI) // endAngle where chart ends (in radians)
	...
	chart.drawKey = function (keyIndicator) {
		...
		var li = list.selectAll("li")
			...
			.append("li").style("cursor", function (d) {
				return (options.PreventLaunch === true && d.data.url !== "other") ? "default" : "pointer";
			});
